...
// --- 1. PRELOADER "CYBER-SCAN" EVOLUTION ---
const Preloader = () => {
  const [progress, setProgress] = useState(0);

  // Simulación de carga de archivos de sistema
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center">
        ...
        {/* Círculo de Carga Hexagonal / Tech */}
        ...
        {/* Texto con Efecto Glitch y Aparición Gradual */}
        <motion.div 
      ...
      {/* Efecto de Escaneo de Pantalla (Barrido horizontal) */}
      <motion.div 
        animate={{ y: ["-100%", "1000%"] }}
        transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
...
// --- 2. FONDO DINÁMICO (PARA TODAS LAS SECCIONES) ---
        ...
        <div className="relative group flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
            {!imageError ? (
              ...
              // FALLBACK: ICONO ORIGINAL SI LA IMAGEN FALLA
              <FaUserTie 
                style={{ fontSize: "12rem", color: "rgba(255,255,255,0.1)" }} 
              />
            )}

            {/* Texto Flotante "Innovación Total" */}
            <motion.div 
              animate={{ y: [0, -10, 0] }} 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        {skills.map((s, i) => (
          ...
          >
            ...
            {/* BARRA DE ENERGÍA INFERIOR ANIMADA */}
...
const Projects = () => {
  ...
  useEffect(() => {
    if (isPaused || selected) return; // Pausa si el mouse está encima o el modal abierto
  ...
  return (
    <section className="py-24 relative overflow-hidden px-6">
      ...
      >
        ...
        {[-1, 0, 1].map((offset) => {
          // Lógica para carrusel infinito circular
          ...
          return (
            <motion.div
              ...
              layout // Suaviza el cambio de posición de los elementos
              initial={{ opacity: 0 }}
              animate={{ 
...
const MemoryGame = () => {
  ...
  // Inicializar juego
  const resetGame = () => {
    const double = [...icons, ...icons].sort(() => Math.random() - 0.5);
    setCards(double);
    setSolved([]);
    setFlipped([]);
  };
  ...
  return (
    <div className="mt-24 p-12 bg-slate-900/40 backdrop-blur-xl rounded-[4rem] border border-blue-500/20 max-w-lg mx-auto shadow-[0_0_50px_rgba(0,0,0,0.5)]">
      {/* TÍTULO ESTILIZADO */}
      <div className="text-center mb-10">
        <h3 className="text-4xl font-black italic bg-gradient-to-r from-white via-blue-400 to-blue-600 bg-clip-text text-transparent uppercase tracking-tighter leading-none">
          Reto de Memoria <br/>
          <span className="text-sm font-mono tracking-[0.3em] text-blue-500/80 mt-2 block">Sincronización de Nodos</span>
        </h3>
      </div>

      {/* GRID DEL JUEGO (20% más grande) */}
        ...
        {cards.map((icon, i) => {
          ...
          return (
            <div 
              ...
              className="relative h-20 w-full perspective-1000 cursor-pointer"
            >
              <motion.div
              ...
              >
                ...
                <div className="absolute inset-0 bg-slate-800 border border-white/10 rounded-2xl flex items-center justify-center text-blue-500/50 text-xl font-black backface-hidden shadow-inner">
                  ?
                </div>
                ...
                <motion.div
                  style={{ rotateY: 180 }}
                  className={`absolute inset-0 flex items-center justify-center rounded-2xl text-3xl backface-hidden shadow-lg ${
        ...
        {solved.length === cards.length && cards.length > 0 && (
          ...
          >
            <p className="text-emerald-400 font-mono text-sm uppercase tracking-widest animate-pulse">
              ¡Sistemas Optimizados con Éxito!
            </p>
            <button 
              onClick={resetGame}
              className="text-[10px] font-black uppercase italic text-white/40 hover:text-blue-400 transition-colors border border-white/10 px-4 py-2 rounded-full"
            >
              Reiniciar Protocolo
            </button>
          </motion.div>
...
const Chatbot = () => {
  ...
  return (
    // AJUSTE POSICIÓN: bottom-28 right-24 (Aproximadamente 25 grados respecto al bottom-10 right-10)
          ...
          >
            ...
            <div className="min-h-[90px] bg-black/50 border border-white/5 p-5 rounded-2xl flex items-center shadow-inner relative overflow-hidden">
              ...
              <AnimatePresence mode="wait">
                {activeFaq && !isTyping && (
                  <motion.p initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-slate-200 text-[12px] leading-relaxed italic font-medium">
                    {activeFaq.a}
                  </motion.p>
                )}
...
<footer className="py-32 border-t border-white/5 relative px-6 overflow-hidden">
  ...
  <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-32">
    ...
    >
{/* ENCABEZADO DE SISTEMA CENTRADO (BLANCO NEÓN) */}
<div className="mb-12 flex flex-col items-center text-center group relative z-10">
  {/* Decoración superior: Puntos de enlace alineados al centro */}
  ...
  />
      ...
      <textarea 
        ...
        className="bg-white/5 p-5 rounded-2xl border border-white/5 text-white outline-none focus:border-blue-500/50 transition-all font-mono text-xs placeholder:text-slate-600"
      ></textarea>
      <button className="group relative bg-white text-black font-black uppercase py-5 rounded-2xl hover:bg-blue-600 hover:text-white transition-all duration-500 overflow-hidden shadow-xl">
  ...
  {/* ANIMACIÓN DE CIERRE - SEPARACIÓN SIGNIFICATIVA */}
  <div className="mt-60 pb-10 relative flex flex-col items-center">
    {/* Línea de escaneo que cruza el ancho */}
    <motion.div 
    ...
    {/* Marca de agua gigante de fondo para el cierre */}
